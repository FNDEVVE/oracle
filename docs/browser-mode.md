...
- `--model gpt-6-pro` (or `gpt-6`, `gpt-6-astra`, `latest`, `pro`): GPT-6 Astra. ChatGPT shows it as the **Latest** model of the advanced picker rather than as a named entry; `gpt-6-pro` and `pro` also select the Pro power tier by default (composer pill "6 Pro"), and Oracle only reports it as selected when that radio is checked or the pill reads "6 …" (never "5.6 …").
